...
/*
  ...
  // 下面的malloc_chunk声明是误导的，(但是是精确和必要的)。它声明了一个内存'视图'：
  // 允许接入必要的域：从一个给定的基数，在已知的偏移位置
  ...
  // 内存chunks用一个`边界tag`方法(起初由Knuth描述)来维护。(看这个paper: ftp://ftp.cs.utexas.edu/pub/garbage/allocsrv.ps)
  // 空闲chunk大小被存储在每个chunk的开头和结尾。这使得碎片化的chunk合并成一个更大的chunks更快。
  // 开头的域持有比特呈现是否chunks是空闲的还是在使用。
  ...
  // 这儿有一些图片能让它更清晰。他们被'分解'去展示chunk的状态，chunk能够被设想作为扩展
  // 从它的头部分的头字段高31位，通过下面的chunk头中的pre_foot和PINUSE_BIT位。
 ...
 mem-> +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  // 注意：自从我们通常合并相邻的空闲chunk，这些chunk相邻的空闲chunk必须正在使用。

  Note that since we always merge adjacent free chunks, the chunks
  adjacent to a free chunk must be in use.

  // 给定一个指向chunk的指针(能够被琐碎地从负载指针获取到)，我们能用O(1)的时间，找到是否
  // 相邻的chunk是否释放，如果是这样，从他们当前的list中解除他们，并用当前chunk合并他们。
  ...
  // chunks通常开始于字边界，所以内存部分(返回给用户的)也开始于字边界，并且因此至少双字对齐。
  // 个人注解: 双字，即8字节，对齐。
  ...
  // P位(PINUSE_BIT)，存储在未使用的低-位chunk大小的位中(通常是两字节的倍数)，对于前一个chunk是一个在使用的位。
  // 如果这个bit被清除，这时这个字节 在当前chunk大小(包含了前一个chunk大小)之前，能被用于查找前面chunk的头。
  // 正好第一个申请的chunk，通常会设置这个位，从而阻止访问不存在的内存。
  // 如果任意给定的chunk设置了pinuse位，这时你不能确定前一个chunk的大小。如果你试着这样做会获取一个无效的内存地址。
  ...
  // C(CINUSE_BIT)位，存储在chunk大小的未使用的第二个低位，冗余记录是否当前chunk是否在使用。
  // 这个冗余在free和remalloc时启用了使用检测，并且当释放和合并chunk时减少了迂回(开销)
  ...
  // 每一个新申请的chunk，必须即设置cinuse也设置pinuse。
  // 因为，每一个申请的chunk相邻的或者是先前申请的，并且一直在使用的chunk，
  // 或者是内存竞技场的base。
  // 这是通过使所有的申请， 确保从任何找到的chunk的最低部分。
  // 更进一步，没有空闲chunk物理邻接另外一个chunk，
  // 因此，每一个空闲chunk的前面和后面要么是在使用的chunk，要么是内存的结尾。
  ...
  // 注意： 当前块的'foot'事实上表示下一个chunk的prev_foot. 这使的更容易地处理对其等，
  // 但是当试着去扩展或者适配这部分代码可能是非常困惑的。
  ...
  // 1. 特定chunk的top是最顶层可用的chunk。(例如：邻接可用内存的结尾的chunk)。
        这种需要特殊处理。
        顶部绝不包含在任何bin，当没有其他chunk是可用的时候，才被使用。
        如果它是非常大(参考M_TRIM_THREM_TRIM_THRESHOLD。
        实际上，顶层的chunk被认为是大于其他任何可用的chunk块(因此小的不太合适)。
        顶层chunk不更新他自己的。
        顶部chunk不会更新他的尾部大小域，因为没有下一个连续chunk需要去索引它。
        然而，对于他(TOP_FOOT_SIZE)来说，当空间被扩展，空间仍然可以申请去启动分割或合并。
  ...
  // 3. chunk通过mmap申请，有最低位(IS_MMAPPED_BIT)设置在他们的prev_foot域。
        并且在他们的头域中不设置PINUSE_BIT。因为他们是一个接着一个的申请的，
        每一个必须携带他们自己的prev_foot域，用于保持这些chunk在他们自己映射的区域的偏移，这是保持对齐必须的。
        每一个映射chunk后面都有一个假的下一个chunk的一个两个字段。为了检测使用情况的目的。
...
//        (15005 & 7) == 0 ?
//          (8 - (15005 & 7)) & 7 = 5
//      alian_as_chunk(A) = (15003 + 5) = 15008
...
/*
  ...
  // 当chunk不再使用，他们不是作为列表的节点就是作为树的节点对待。

  "Small"  chunks are stored in circular doubly-linked lists, and look
  like this:

  // '小' chunks 用来存储循环双链表，看起来如下：
...
nextchunk-> +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  // 大chunk用位数树，以chunk的大小为key的形式 保存。因为malloc_tree_chunks仅仅用于大于等于
    256字节的空闲chunk，他们的大小并不强制限制用户的chunk大小。每个节点看起来如下：
...
nextchunk-> +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  ...
  // 每一颗包含树节点的树，是一个唯一chunk大小的树。 相同大小的chunk被分配在一个循环链表中，
  // 树中实际上仅有最老的chunk(在我们的FIFO顺序中，下一个是使用的)。
  // (树成员通过一个非空父指针区分) 如果具有相同大小的区块，则插入现有节点，
  // 它使用与小数据块的fd/bk指针工作方式相同的指针与现有节点链接。
